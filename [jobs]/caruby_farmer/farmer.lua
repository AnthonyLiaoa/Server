	...
	while true do
		...
		if GetDistanceBetweenCoords(coords, config.zones.process.coords, true) < 1 then
			if not isProcessing then
				ESX.ShowHelpNotification("按 ~INPUT_CONTEXT~ ~g~處理稻穗")
			end
	...
	while true do
		...
		if nearbyObject then
					if not isPickingUp and GetDisplayNameFromVehicleModel(GetEntityModel(veh)) == "TRACTOR" then
						ESX.ShowHelpNotification("按 ~INPUT_THROW_GRENADE~  ~g~收割")
					end
...
-- 				ESX.ShowHelpNotification("按 ~INPUT_CONTEXT~ ~g~生成工作車輛")
-- 				if not Tractor then	
...
function SpawnProps()
	while spawnedProps < 25 do
		...
		ESX.Game.SpawnLocalObject('prop_coral_spikey_01', Coords, function(obj)
