			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				if closestPlayer ~= -1 and closestDistance <= 3.0 then
					...
					if action == 'identity_card' then
						...
						TriggerServerEvent('esx_policejob:message', GetPlayerServerId(closestPlayer), _U('being_searched_identifier', GetPlayerServerId(PlayerId())))
					elseif action == 'body_search' then
						TriggerServerEvent('esx_policejob:message', GetPlayerServerId(closestPlayer), _U('being_searched', GetPlayerServerId(PlayerId())))
	...
	ESX.TriggerServerCallback('esx_policejob:getFineList', function(fines)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'fine_category', {
			...
			if Config.EnablePlayerManagement then
				...
				TriggerServerEvent("azk_discordlogs:PoliceLogs", 'fine', GetPlayerServerId(PlayerId()), "", data.current.amount, GetPlayerServerId(player))
			else
...
function TrackVehicle()
	...
	function(data, menu)
		...
		if data.value == nil or length < 2 or length > 13 then
			...
			TriggerEvent('esx_policejob:searchPlate', data.value)
			menu.close()
		end
	end, function(data, menu)
		menu.close()
...
AddEventHandler('esx_policejob:hasEnteredEntityZone', function(entity)
	...
	if PlayerData.job.name ~= 'police' and GetEntityModel(entity) == GetHashKey('p_ld_stinger_s') then
		local playerPed = PlayerPedId()
		local coords    = GetEntityCoords(playerPed)
...
RegisterNetEvent('esx_policejob:OpenPoliceActionsMenu')
...
RegisterNetEvent("esx_policejob:updateTimer")
AddEventHandler("esx_policejob:updateTimer", function(plate)
    time_out[plate] = time_out[nil]
end)

RegisterNetEvent("esx_policejob:updateActivePlate")
AddEventHandler("esx_policejob:updateActivePlate", function(plate)
...
RegisterNetEvent("esx_policejob:getActivePlates")
AddEventHandler("esx_policejob:getActivePlates", function(plates)
    time_out = plates
    for v,k in pairs(time_out) do
        checkVehicle(v)
    end
...
RegisterNetEvent('esx_policejob:plate')
AddEventHandler('esx_policejob:plate', function(plate)
    checkVehicle(plate)
end)

AddEventHandler('esx_policejob:searchPlate', function(plate, cb)
    ...
    for i=1, #vehicle, 1 do 
        ...
        if vehicleProps.plate == plate then 
			...
			TriggerServerEvent("esx_policejob:tracker", plate)
        else
            miss = miss + 1
        end
    end
...
AddEventHandler("esx_policejob:removeSearch", function(plate, cb)
    local vehicle = ESX.Game.GetVehicles()
    local miss = 0
    ...
    for i=1, #vehicle, 1 do 
        ...
        if vehicleProps.plate == plate then 
			TriggerServerEvent("esx_policejob:removeActivePlate", plate)
...
function checkVehicle(plate)
    ...
    for i=1, #vehicle, 1 do 
        ...
        if vehicleProps.plate == plate then 
            TriggerServerEvent("esx_policejob:setActivePlates", plate)
...
function triggerTimer(plate)
    TriggerServerEvent("esx_policejob:triggerTimer", plate)
end
...
function OpenBlackMoneyMenu()
	ESX.TriggerServerCallback("esx_policejob:getSharedAccount", function(amount)
		ESX.UI.Menu.Open("default", GetCurrentResourceName(), "police_black_money",
		{
			title = "黑錢選單",
			elements = {
				{label = "黑錢: $"..amount},
			...
			if data.current.value == "get" then
				ESX.UI.Menu.Open("dialog", GetCurrentResourceName(), "get_black_money",
				{
					...
					if tonumber(data2.value) > 0 then
						TriggerServerEvent("esx_policejob:getBlackMoney", tonumber(data2.value))
						menu2.close()
						OpenBlackMoneyMenu()
					else
			...
			elseif data.current.value == "put" then
				ESX.UI.Menu.Open("dialog", GetCurrentResourceName(), "put_black_money",
				{
					...
					if tonumber(data2.value) > 0 then
						TriggerServerEvent("esx_policejob:putBlackMoney", tonumber(data2.value))
						menu2.close()
						OpenBlackMoneyMenu()
					else
						ESX.ShowNotification("數量錯誤")
