	...
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_mechanic', function(inventory)
		...
		if count > 0 and item.count >= count then
			...
			if sourceItem.limit ~= -1 and (sourceItem.count + count) > sourceItem.limit then
				...
				TriggerEvent('azk_discordlogs:MechanicLogs', 'get', source, item.label, count, "")
			end
		else
			TriggerClientEvent('esx:showNotification', xPlayer.source, _U('invalid_quantity'))
			TriggerEvent('mk:log', {
				type = 'mechanicgetitem',
				title = '修車廠倉庫放入紀錄',
				content = ('%s x%s'):format(ESX.Items[itemName].label, count),
				players = {_source, target},
				color = 'green'
			})
		end
	end)
...
AddEventHandler('esx_mechanicjob:putStockItems', function(itemName, count)
	...
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_mechanic', function(inventory)
		...
		if item.count >= 0 and count <= playerItemCount then
			...
			TriggerEvent('azk_discordlogs:MechanicLogs', 'put', _source, item.label, count, "")
		else
			TriggerClientEvent('esx:showNotification', xPlayer.source, _U('invalid_quantity'))
		end
...
ESX.RegisterServerCallback("esx_mechanicjob:getFineList", function(source, cb, type)

	MySQL.Async.fetchAll("SELECT label, amount FROM fine_type_mechanic WHERE type = @type", {
		["@type"] = type
	}, function(result)
		local fine = {}
		
		for i = 1, #result, 1 do
			table.insert(fine, {label = result[i].label, amount = result[i].amount})
		end
...
ESX.RegisterServerCallback('esx_mechanicjob:buyJobVehicle', function(source, cb, vehicleProps)
	local xPlayer = ESX.GetPlayerFromId(source)
	local price = getPriceFromHash(vehicleProps.model, xPlayer.job.grade_name)
...
function getPriceFromHash(hashKey, jobGrade)
