			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'billing', {
				...
				if amount == nil then
					...
					if closestPlayer == -1 or closestDistance > 3.0 then
						...
						TriggerServerEvent('azk_discordlogs:AmbulanceLogs', 'fine', GetPlayerServerId(PlayerId()), "", amount, GetPlayerServerId(closestPlayer))
						ESX.ShowNotification(_U('billing_sent'))
						TriggerServerEvent("dclog:ambulancebill", GetPlayerServerId(closestPlayer), amount)
					end
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				if closestPlayer == -1 or closestDistance > 1.0 then
					...
					elseif data.current.value == 'identity_card' then
						...
						TriggerServerEvent('esx_ambulancejob:message', GetPlayerServerId(closestPlayer), _U('being_card', GetPlayerServerId(PlayerId())))
					end
				end
			end, function(data, menu)
				menu.close()
...
Citizen.CreateThread(function()
	while true do
		...
		for hospitalNum,hospital in pairs(Config.Hospitals) do
			...
			--Cloakrooms
			for k,v in ipairs(hospital.Cloakrooms) do
				local distance = GetDistanceBetweenCoords(playerCoords, v, true)
				...
				if distance < Config.Marker.x then
					isInMarker, currentHospital, currentPart, currentPartNum = true, hospitalNum, 'Cloakrooms', k
				end
			end
...
AddEventHandler('esx_ambulancejob:hasEnteredMarker', function(hospital, part, partNum)
	if ESX.PlayerData.job and ESX.PlayerData.job.name == 'ambulance' then
		...
		elseif part == 'Cloakrooms' then
			CurrentAction = part
			CurrentActionMsg = _U('open_cloakrooms')
			CurrentActionData = {}
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, Keys['E']) then
				...
				elseif CurrentAction == 'Pharmacy' then
					OpenPharmacyMenu()
				elseif CurrentAction == 'Cloakrooms' then
					OpenCloakroomMenu()
...
RegisterNetEvent('esx_ambulancejob:OpenAmbulanceActionsMenu')
AddEventHandler('esx_ambulancejob:OpenAmbulanceActionsMenu', function()
	OpenMobileAmbulanceActionsMenu()
end)
