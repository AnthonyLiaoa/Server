	...
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_grove', function(inventory)
		...
		if count > 0 and inventoryItem.count >= count then
			...
			if sourceItem.limit ~= -1 and (sourceItem.count + count) > sourceItem.limit then
				...
				TriggerEvent('azk_discordlogs:GroveLogs', 'get', xPlayer.source, 0, inventoryItem.label, count, "")
			end
		else
			TriggerClientEvent('esx:showNotification', _source, _U('quantity_invalid'))
		end
...
AddEventHandler('esx_grovejob:putStockItems', function(itemName, count)
	...
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_grove', function(inventory)
		...
		if sourceItem.count >= count and count > 0 then
			...
			TriggerEvent('azk_discordlogs:GroveLogs', 'put', xPlayer.source, 0, inventoryItem.label, count, "")
		else
...
ESX.RegisterServerCallback('esx_grovejob:addArmoryWeapon', function(source, cb, weaponName, removeWeapon)
	...
	TriggerEvent('esx:groveJobItemDra', 'grove', xPlayer.name, weaponLabel, 500, '放入')
...
ESX.RegisterServerCallback('esx_grovejob:removeArmoryWeapon', function(source, cb, weaponName)
	...
	local weaponLabel = ESX.GetWeaponLabel(weaponName)
	TriggerEvent('esx:groveJobItemDra', 'grove', xPlayer.name, weaponLabel, 500, '取出')
	TriggerEvent('esx_datastore:getSharedDataStore', 'society_grove', function(store)
...
RegisterServerEvent('esx_grovejob:requestarrest')
AddEventHandler('esx_grovejob:requestarrest', function(targetid, playerheading, playerCoords,  playerlocation)
    _source = source
    TriggerClientEvent('esx_grovejob:getarrested', targetid, playerheading, playerCoords, playerlocation)
    TriggerClientEvent('esx_grovejob:doarrested', _source)
end)

RegisterServerEvent('esx_grovejob:requestrelease')
AddEventHandler('esx_grovejob:requestrelease', function(targetid, playerheading, playerCoords,  playerlocation)
    _source = source
    TriggerClientEvent('esx_grovejob:getuncuffed', targetid, playerheading, playerCoords, playerlocation)
    TriggerClientEvent('esx_grovejob:douncuffing', _source)
end)
