	...
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_taxi', function(inventory)
		...
		if count > 0 and item.count >= count then
			...
			if sourceItem.limit ~= -1 and (sourceItem.count + count) > sourceItem.limit then
				...
				TriggerEvent('azk_discordlogs:UberLogs', 'get', source, item.label, count, "")
			end
		else
...
AddEventHandler('esx_taxijob:putStockItems', function(itemName, count)
	...
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_taxi', function(inventory)
		...
		if item.count >= 0 and count <= sourceItem.count then
			xPlayer.removeInventoryItem(itemName, count)
			inventory.addItem(itemName, count)
			TriggerClientEvent('esx:showNotification', xPlayer.source, _U('have_deposited', count, item.label))
			TriggerEvent('azk_discordlogs:UberLogs', 'put', xPlayer.source, item.label, count, "")
		else
			TriggerClientEvent('esx:showNotification', xPlayer.source, _U('quantity_invalid'))
		end
...
ESX.RegisterServerCallback('esx_taxijob:buyJobVehicle', function(source, cb, vehicleProps)
	local xPlayer = ESX.GetPlayerFromId(source)
