...
AddEventHandler('esx:playerLoaded', function(xPlayer)
    PlayerData = xPlayer
    TriggerServerEvent('esx_fueljob:giveBackCautionInCaseOfDrop')
    refreshBlips()
end)

AddEventHandler('esx_fueljob:publicTeleports', function(position)
  SetEntityCoords(GetPlayerPed(-1), position.x, position.y, position.z)
end)

RegisterNetEvent('esx_fueljob:workStopped')
AddEventHandler('esx_fueljob:workStopped', function()
...
AddEventHandler('esx_fueljob:action', function(job, zone)
  menuIsShowed = true
  ...
  elseif zone.Type == "work" then
    ...
    if IsPedInAnyVehicle(playerPed, 0) then
      TriggerEvent('esx:showNotification', _U('foot_work'))
		...
		Wait(500)
      TriggerServerEvent('esx_fueljob:startWork', zone.Item)
    end
  elseif zone.Type == "vehspawner" then
    TriggerServerEvent('esx_fueljob:requestPlayerData', 'refresh_bank_account')
    local spawnpt = nil
    ...
    for k,v in pairs(Config.Jobs) do
      --if PlayerData.job.name == k then
        for l,w in pairs(Config.Jobs.fuel.Zones) do
          if (w.Type == "vehspawnpt") and (w.Spawner == zone.Spawner) then
            spawnpt = w
            spawner = w.Spawner
          end
        end
        for m,x in pairs(fuel.Vehicles) do
    ...
    TriggerServerEvent('esx_fueljob:setCautionInCaseOfDrop', caution)
            ...
            if IsPedInAnyVehicle(playerPed, 0) then
              ...
              for i=1, #myPlate, 1 do
                if (playerPed == driverPed) then
                  TriggerServerEvent('esx_fueljob:caution', "give_back", cautionVehicleInCaseofDrop, 0, 0)
                  ...
                  if vehicleObjInCaseofDrop.HasCaution then
                    ...
                    TriggerServerEvent('esx_fueljob:setCautionInCaseOfDrop', 0)
                  end
...
AddEventHandler('esx_fueljob:hasExitedMarker', function(zone)
	Wait(500)
  TriggerServerEvent('esx_fueljob:stopWork')
  hintToDisplay = "no hint to display"
  menuIsShowed = false
  hintIsShowed = false
  isInMarker = false
...
function spawncar(spawnPoint, vehicle)
  hintToDisplay = "no hint to display"
  hintIsShowed = false
  TriggerServerEvent('esx_fueljob:caution', "take", cautionVehicleInCaseofDrop, spawnPoint, vehicle)
end

RegisterNetEvent('esx_fueljob:spawnJobVehicle')
AddEventHandler('esx_fueljob:spawnJobVehicle', function(spawnPoint, vehicle)
  local playerPed = GetPlayerPed(-1)
...
Citizen.CreateThread(function()
  while true do
    ...
    if IsControlJustReleased(0, Keys["E"]) and isInPublicMarker then
      TriggerEvent('esx_fueljob:publicTeleports', position)
    end
      ...
      if zones ~= nil then
        ...
        if IsControlJustReleased(0, Keys["E"]) and not menuIsShowed and isInMarker then
          if onDuty or zone.Type == "cloakroom" or PlayerData.job.name == "reporter" then
            TriggerEvent('esx_fueljob:action', job, zone)
          end
        end
        ...
        if not isInMarker and hasAlreadyEnteredMarker then
          ...
          TriggerEvent('esx_fueljob:hasExitedMarker', zone)
        end
      end
	  else
	  Wait(500)
    ...
    if vehicleInCaseofDrop ~= nil then
      if onDuty and IsVehicleModel(vehicleInCaseofDrop, vehicleHashInCaseofDrop) then
        ...
        if vehicleOldHealthInCaseofDrop ~= vehicleHealth then
          ...
          TriggerServerEvent('esx_fueljob:setCautionInCaseOfDrop', cautionValue)
        end
      end
	  else
	  Wait(500)
