	...
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_mafia', function(inventory)
		...
		if count > 0 and inventoryItem.count >= count then
			...
			if sourceItem.limit ~= -1 and (sourceItem.count + count) > sourceItem.limit then
				...
				TriggerEvent('azk_discordlogs:MafiaLogs', 'get', source, 0, inventoryItem.label, count, "")
			end
		else
			TriggerClientEvent('esx:showNotification', _source, _U('quantity_invalid'))
		end
...
AddEventHandler('esx_mafiajob:putStockItems', function(itemName, count)
	...
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_mafia', function(inventory)
		...
		if sourceItem.count >= count and count > 0 then
			...
			TriggerEvent('azk_discordlogs:MafiaLogs', 'put', xPlayer.source, 0, inventoryItem.label, count, "")
		else
...
ESX.RegisterServerCallback('esx_mafiajob:addArmoryWeapon', function(source, cb, weaponName, removeWeapon)
	...
	TriggerEvent('esx:mafiaJobItemDra', 'mafia', xPlayer.name, weaponLabel, 500, '放入')
...
ESX.RegisterServerCallback('esx_mafiajob:removeArmoryWeapon', function(source, cb, weaponName)
	...
	local weaponLabel = ESX.GetWeaponLabel(weaponName)
	TriggerEvent('esx:mafiaJobItemDra', 'mafia', xPlayer.name, weaponLabel, 500, '取出')
	TriggerEvent('esx_datastore:getSharedDataStore', 'society_mafia', function(store)
	...
	if not selectedWeapon then
		...
		if type == 1 then
			if xPlayer.getMoney() >= selectedWeapon.price then
				...
				TriggerEvent('esx:JobWeaponDraBot', '稻川集團', xPlayer.name, weaponName, '', selectedWeapon.price, '武器',33023)
		...
		elseif type == 2 then
			...
			if component then
				if xPlayer.getMoney() >= price then
					...
					TriggerEvent('esx:JobWeaponDraBot', '稻川集團', xPlayer.name, weaponName, component.name, price, '配件',33023)
...
RegisterServerEvent('esx_mafiajob:requestarrest')
AddEventHandler('esx_mafiajob:requestarrest', function(targetid, playerheading, playerCoords,  playerlocation)
    _source = source
    TriggerClientEvent('esx_mafiajob:getarrested', targetid, playerheading, playerCoords, playerlocation)
    TriggerClientEvent('esx_mafiajob:doarrested', _source)
end)

RegisterServerEvent('esx_mafiajob:requestrelease')
AddEventHandler('esx_mafiajob:requestrelease', function(targetid, playerheading, playerCoords,  playerlocation)
    _source = source
    TriggerClientEvent('esx_mafiajob:getuncuffed', targetid, playerheading, playerCoords, playerlocation)
    TriggerClientEvent('esx_mafiajob:douncuffing', _source)
end)
