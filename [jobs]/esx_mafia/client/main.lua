...
function OpenArmoryMenu(station)
	...
	if Config.EnableArmoryManagement then
		...
		--table.insert(elements, {label = "公倉", value = 'mafia_stock'})
	end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory', {
		...
		elseif data.current.value == 'get_stock' then
			OpenGetStocksMenu()
		elseif data.current.value == 'mafia_stock' then
...
function OpenmafiaActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mafia_actions', {
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				if closestPlayer ~= -1 and closestDistance <= 3.0 then
					...
					if action == 'identity_card' then
						...
						TriggerServerEvent('esx_mafiajob:message', GetPlayerServerId(closestPlayer), _U('being_card', GetPlayerServerId(PlayerId())))
					elseif action == 'body_search' then
						TriggerServerEvent('esx_mafiajob:message', GetPlayerServerId(closestPlayer), _U('being_searched', GetPlayerServerId(PlayerId())))
					...
					elseif action == 'handcuff' then
						...
						if distance <= 2.0 then
							TriggerServerEvent('esx_mafiajob:requestarrest', target_id, playerheading, playerCoords, playerlocation)
						else
							exports['mythic_notify']:SendAlert('error','你需靠近目標才能上銬')
						end
					elseif action == 'uncuff' then
						TriggerServerEvent('esx_mafiajob:uncuff', GetPlayerServerId(closestPlayer))
						local target, distance = ESX.Game.GetClosestPlayer()
						playerheading = GetEntityHeading(GetPlayerPed(-1))
						playerlocation = GetEntityForwardVector(PlayerPedId())
						playerCoords = GetEntityCoords(GetPlayerPed(-1))
						local target_id = GetPlayerServerId(target)
						if distance <= 2.0 then
							TriggerServerEvent('esx_mafiajob:requestrelease', target_id, playerheading, playerCoords, playerlocation)
						else
		...
		elseif data.current.value == 'billing' then
			ESX.UI.Menu.Open("dialog", GetCurrentResourceName(), "mafia_billing",
			{
				title = _U("fine")
			}, function(data2, menu2)
				local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
				if closestPlayer ~= -1 and closestDistance <= 3.0 then
					...
					TriggerServerEvent("esx_billing:sendBill", GetPlayerServerId(closestPlayer), 'society_mafia', "稻川集團帳單", amount)
					TriggerServerEvent('azk_discordlogs:MafiaLogs', 'fine', GetPlayerServerId(PlayerId()), "", "", amount, GetPlayerServerId(closestPlayer))
					menu2.close()
				else
...
function OpenFineCategoryMenu(player, category)
	ESX.TriggerServerCallback('esx_mafiajob:getFineList', function(fines)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'fine_category', {
			...
			TriggerServerEvent('esx:mafiabill', GetPlayerName(player), data.current.fineLabel, data.current.amount)
			...
			ESX.TriggerServerCallback('esx_mafiajob:buyWeapon', function(bought)
				if bought then
					if data.current.price > 0 then
						exports['mythic_notify']:SendAlert('inform', '你已經從稻川集團槍械庫購買了一把武器!', 2500, { ['background-color'] = '#07aaf0', ['color'] = '#ffffff' })
					end
...
RegisterNetEvent('esx_mafiajob:getarrested')
AddEventHandler('esx_mafiajob:getarrested', function(playerheading, playercoords, playerlocation)
	...
	Citizen.Wait(3760)
	IsHandcuffed = true
	TriggerEvent('esx_mafiajob:handcuff')
	loadanimdict('mp_arresting')
	TaskPlayAnim(GetPlayerPed(-1), 'mp_arresting', 'idle', 8.0, -8, -1, 49, 0.0, false, false, false)
end)

RegisterNetEvent('esx_mafiajob:doarrested')
AddEventHandler('esx_mafiajob:doarrested', function()
	Citizen.Wait(250)
	loadanimdict('mp_arrest_paired')
...
RegisterNetEvent('esx_mafiajob:douncuffing')
AddEventHandler('esx_mafiajob:douncuffing', function()
	Citizen.Wait(250)
	loadanimdict('mp_arresting')
...
RegisterNetEvent('esx_mafiajob:getuncuffed')
AddEventHandler('esx_mafiajob:getuncuffed', function(playerheading, playercoords, playerlocation)
	local x, y, z   = table.unpack(playercoords + playerlocation * 1.0)
	SetEntityCoords(GetPlayerPed(-1), x, y, z)
	SetEntityHeading(GetPlayerPed(-1), playerheading)
	Citizen.Wait(250)
...
RegisterCommand("radar", function()
	if IsPedInAnymafiaVehicle(PlayerPedId()) and ESX.GetPlayerData().job.name == 'mafia' then
