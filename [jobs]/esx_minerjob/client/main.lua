    ...
    Citizen.CreateThread(function()
        while true do
            ...
            if GetDistanceBetweenCoords(GetEntityCoords(ped), Config.WashingX, Config.WashingY, Config.WashingZ, true) < 5 then
                Draw3DText( Config.WashingX, Config.WashingY, Config.WashingZ+0.5 -1.400, ('正在清洗原礦:' .. timer .. '%'), 0, 0.1, 0.1)
            end
            if GetDistanceBetweenCoords(GetEntityCoords(ped), Config.RemeltingX, Config.RemeltingY, Config.RemeltingZ, true) < 5 then
                Draw3DText( Config.RemeltingX, Config.RemeltingY, Config.RemeltingZ+0.5 -1.400, ('正在熔煉礦石:' .. timer .. '%'), 0, 0.1, 0.1)
            end
            if timer == 100 then
                timer = 0
                break
...
Citizen.CreateThread(function()
    ...
    SetBlipAsShortRange(blip1, true)
    BeginTextCommandSetBlipName("STRING")
    AddTextComponentString("礦場|1-更衣室")
    EndTextCommandSetBlipName(blip1)   
end)
        ...
        if PlayerData.job ~= nil and PlayerData.job.name == 'miner' and not IsEntityDead( ped ) then
            if GetDistanceBetweenCoords(GetEntityCoords(ped), Config.CloakroomX, Config.CloakroomY, Config.CloakroomZ, true) < 25 then
                ...
                if GetDistanceBetweenCoords(GetEntityCoords(ped), Config.CloakroomX, Config.CloakroomY, Config.CloakroomZ, true) < 1 then
                    ESX.ShowHelpNotification('按 ~INPUT_CONTEXT~ 開啟更衣室')
                    if IsControlJustReleased(1, 38) then
                        if jobname == 'miner' then
                            Cloakroom() 
                        else
                            ESX.ShowNotification("~r~你不是礦工!!!~w~")
                        end
                    end
                end
            end

            if GetDistanceBetweenCoords(GetEntityCoords(ped), Config.DeleteVehicleX, Config.DeleteVehicleY, Config.DeleteVehicleZ, true) < 25 then
                DrawMarker(1, Config.DeleteVehicleX, Config.DeleteVehicleY, Config.DeleteVehicleZ, 0, 0, 0, 0, 0, 90.0, 2.8, 2.8, 1.5, 255, 0, 0, 155, 0, 0, 2, 0, 0, 0, 0)
                if GetDistanceBetweenCoords(GetEntityCoords(ped), Config.DeleteVehicleX, Config.DeleteVehicleY, Config.DeleteVehicleZ, true) < 2.6 then
                    ESX.ShowHelpNotification('按 ~INPUT_CONTEXT~ 還回工作車')
                    if IsControlJustReleased(1, 38) then
                        if jobname == 'miner' then
                            DeleteVehicle() 
                        else
    ...
    while true do
        ...
        if GetDistanceBetweenCoords(GetEntityCoords(ped), Config.WashingX, Config.WashingY, Config.WashingZ, true) < 25 and washingActive == false then
            ...
            if GetDistanceBetweenCoords(GetEntityCoords(ped), Config.WashingX, Config.WashingY, Config.WashingZ, true) < 1 then
                ESX.ShowHelpNotification("按 ~INPUT_CONTEXT~ 開始洗礦")
                if IsControlJustReleased(1, 38) then
                    if jobname == 'miner' then
        ...
        if GetDistanceBetweenCoords(GetEntityCoords(ped), Config.RemeltingX, Config.RemeltingY, Config.RemeltingZ, true) < 25 and remeltingActive == false then
            ...
            if GetDistanceBetweenCoords(GetEntityCoords(ped), Config.RemeltingX, Config.RemeltingY, Config.RemeltingZ, true) < 1 then
                ESX.ShowHelpNotification("按 ~INPUT_CONTEXT~ 開始熔煉礦石")
                if IsControlJustReleased(1, 38) then
                    if jobname == 'miner' then
        ...
        if GetDistanceBetweenCoords(GetEntityCoords(ped), Config.SellX, Config.SellY, Config.SellZ, true) < 2 then
            ESX.ShowHelpNotification("按 ~INPUT_CONTEXT~ 販賣礦石")
            if IsControlJustReleased(1, 38) then
                if jobname == 'miner' then
                    Jeweler()  
                else
        ...
        for i=1, #locations, 1 do
            if GetDistanceBetweenCoords(GetEntityCoords(ped), locations[i].x, locations[i].y, locations[i].z, true) < 25 and mineActive == false then
                ...
                if GetDistanceBetweenCoords(GetEntityCoords(ped), locations[i].x, locations[i].y, locations[i].z, true) < 1 then
                    ESX.ShowHelpNotification("按 ~INPUT_CONTEXT~ 開始採礦")
                    if IsControlJustReleased(1, 38) then
                        if jobname == 'miner' then
                            Animation()
                            mineActive = true
...
Citizen.CreateThread(function()
    ...
    if firstspawn == false then
        ...
        print(Config.SellY)
        print(Config.SellZ)
        local npc = CreatePed(1, hash, Config.SellX, Config.SellY, Config.SellZ, Config.SellH, false, false)
...
function Cloakroom()
    local elements = {
        {label = '便服',   value = 'cloakroom1'},
        {label = '工作服',      value = 'cloakroom2'},
        {label = '工作車',       value = 'vehicle'},
    }
    ...
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'miner_actions', {
        ...
        elseif data.current.value == 'vehicle' then
            menu.close()
            RequestModel(Config.WorkVehicle)
            Citizen.Wait(100)
            CreateVehicle(Config.WorkVehicle, 873.53, -2196.58, 29.52, 0.0, true, true)
            ESX.ShowNotification("工作車已駛出")
        end
...
function DeleteVehicle()
    ...
    if GetEntityModel(vehicle) == GetHashKey(Config.WorkVehicle) then
        ESX.Game.DeleteVehicle(vehicle)
    else
        ESX.ShowNotification('這不是工作車')
    end
...
function Jeweler()
    local elements = {
        {label = '賣鑽石',   value = 'diamonds'},
        {label = '賣黃金',      value = 'gold'},
        {label = '賣鐵礦',       value = 'iron'},
        {label = '賣銅礦',       value = 'copper'},
    }
