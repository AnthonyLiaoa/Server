  ...
  local identifier =  ESX.GetPlayerFromId(src).identifier
        ...
        function(data)
            ...
            if count > data[1].count then
                ...
                TriggerClientEvent('esx:showNotification', xPlayer.source, '~r~您不能比自己擁有更多的東西')
                else
    ...
    function(items)
      ...
      MySQL.Async.fetchAll(
        ...
        if data[1] == nil then
            ...
            imgsrc = 'nui://esx_inventoryhud/html/img/items/'..Item..'.png'
            ...
            elseif data ~= nil and data[1].price ~= Price then
                Wait(250)
                TriggerClientEvent('esx:showNotification', xPlayer.source, '~r~您的商店中已經有相同的商品，~r~但是' .. data[1].price .. '你把價格' .. Price)
                Wait(250)
                TriggerClientEvent('esx:showNotification', xPlayer.source, '~r~刪除商品並標上新價格或同價')
            end
        end)  
    end)
end)
...
AddEventHandler('esx_kr_shops:Buy', function(id, Item, ItemCount)
  ...
  local sourceItem = xPlayer.getInventoryItem(Item)
        ...
        MySQL.Async.fetchAll(
        'SELECT price, item, count FROM shops WHERE ShopNumber = @Number AND item = @item',
        {
            ['@Number'] = id,
            ['@item'] = Item,
        }, function(result)
        ...
        'SELECT money FROM owned_shops WHERE ShopNumber = @Number',
        {
            ...
            if xPlayer.getMoney() < ItemCount * result[1].price then
                TriggerClientEvent('esx:showNotification', src, '~r~您沒有足夠的錢。')
            elseif ItemCount <= 0  or result[1].count < ItemCount or (sourceItem.count + ItemCount > sourceItem.limit) then
                TriggerClientEvent('esx:showNotification', src, '~r~無效數量。')
            else
                xPlayer.removeMoney(ItemCount * result[1].price)
                TriggerClientEvent('esx:showNotification', xPlayer.source, '~g~你買了' .. ItemCount .. 'x ' .. Item .. '花了$' .. ItemCount * result[1].price)
                xPlayer.addInventoryItem(result[1].item, ItemCount)
                TriggerEvent("azk_discordlogs:BuyItem", xPlayer.source, sourceItem.label, ItemCount)
...
AddEventHandler('esx_kr_shops:GetAllItems', function(id)
    ...
    MySQL.Async.fetchAll(
    'SELECT item, count FROM shipments WHERE id = @id AND identifier = @identifier',
    {
        ['@id'] = id,
        ['@identifier'] = identifier
    }, function(result)
  ...
  local identifier = ESX.GetPlayerFromId(_source).identifier
    ...
    MySQL.Async.fetchAll('SELECT money FROM owned_shops WHERE ShopNumber = @ShopNumber AND identifier = @identifier',{['@ShopNumber'] = id,['@identifier'] = identifier,}, function(result)
        ...
        if result[1].money >= price * count then
            ...
            TriggerClientEvent('esx:showNotification', _source, '~r~您的商店沒有足夠的錢。')
        end
    end)
end)
...
local identifier = ESX.GetPlayerFromId(source).identifier
    ...
    if result[1].identifier == '0' then
        ...
        if xPlayer.getMoney() >= price then
            ...
            TriggerClientEvent('esx:showNotification', _source, '~g~您以$購買了一家商店' ..  price)
        else    
            TriggerClientEvent('esx:showNotification', _source, '~r~你買不起這家商店')
        end
...
local identifier = ESX.GetPlayerFromId(source).identifier
    ...
    MySQL.Async.fetchAll(
        'SELECT LastRobbery, money FROM owned_shops WHERE identifier = @identifier AND ShopNumber = @Number',
        {
          ['@identifier'] = identifier,
          ['@Number'] = number,
        },
        ...
        if os.time() - result[1].LastRobbery <= 900 then
            ...
            TriggerClientEvent('esx:showNotification', xPlayer.source, '~r~您的店鋪資金因搶劫而被鎖定，請稍候' .. math.floor((900 - time) / 60) .. ' 分鐘')
            return
        end
        ...
        TriggerClientEvent('esx:showNotification', xPlayer.source, '~g~您輸入了$' .. amount .. '在你的店裡')
        else
        TriggerClientEvent('esx:showNotification', xPlayer.source, '~r~您不能投入超過自己的能力')
        end
    end)
end)
  ...
  MySQL.Async.fetchAll(
    ...
    if os.time() - result[1].LastRobbery <= 900 then
        ...
        if result[1].money >= amount then
            ...
            TriggerClientEvent('esx:showNotification', xPlayer.source, '~g~你拿出$' .. amount .. '從你的商店')
...
AddEventHandler('esx_kr_shops:SellShop', function(number)
  ...
  local xPlayer = ESX.GetPlayerFromId(src)
  MySQL.Async.fetchAll(
    'SELECT money, ShopValue FROM owned_shops WHERE identifier = @identifier AND ShopNumber = @ShopNumber',
    {
      ['@identifier'] = identifier,
      ['@ShopNumber'] = number,
    },
    function(result)
      MySQL.Async.fetchAll(
        'SELECT item FROM shops WHERE ShopNumber = @ShopNumber',
        {
          ['@ShopNumber'] = number,
        },
        function(result2)
      ...
      if result[1].money == 0 and result2[1] == nil then
        MySQL.Async.fetchAll("UPDATE owned_shops SET identifier = @identifiers, ShopName = @ShopName WHERE identifier = @identifier AND ShopNumber = @Number",
        {
            ...
            TriggerClientEvent('esx_kr_shops:removeBlip', -1)
            TriggerClientEvent('esx_kr_shops:setBlip', -1)
            TriggerClientEvent('esx:showNotification', xPlayer.source, '~g~你賣了你的店')
            else
            TriggerClientEvent('esx:showNotification', xPlayer.source, '~r~您不能在商店內出售物品或金錢')
            end
        end)
    end)
end)
...
ESX.RegisterServerCallback('esx_kr_shop-robbery:getUpdates', function(source, cb, id)
    ...
    MySQL.Async.fetchAll(
    'SELECT LastRobbery, ShopName FROM owned_shops WHERE ShopNumber = @ShopNumber',
    {
     ['@ShopNumber'] = id,
    },
     function(result)
...
AddEventHandler('esx_kr_shops-robbery:GetReward', function(id)
    ...
    MySQL.Async.fetchAll('SELECT money FROM owned_shops WHERE ShopNumber = @ShopNumber',
    {
        ...
        TriggerClientEvent("esx:showNotification", xPlayer.source, "你搶到了 ~y~$"..result[1].money / Config.CutOnRobbery.."~s~")
    end)
end)
...
RegisterServerEvent('esx_kr_shops-robbery:startRobbing')
AddEventHandler('esx_kr_shops-robbery:startRobbing', function(stores)
    TriggerClientEvent("esx_kr_shops-robbery:policenotify", -1, stores)
    TriggerEvent('azk_discordlogs:RobberyLogs', 'shop', source, stores)
end)
