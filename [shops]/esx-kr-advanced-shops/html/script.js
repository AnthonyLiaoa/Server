			...
			for (i = 0; i < event.data.result.length; i++) {
				...
				$('#wrapper').append(
					`<div class = "image" id = ${event.data.result[i].item} label = ${event.data.result[i].label} count = ${event.data.result[i].count} price = ${event.data.result[i].price}>
						...
						<h5 class = "h4">庫存: ${event.data.result[i].count}x</h5>
					</div>`
				);
			}

			$('#wrapper').append(
				` <h6 class = "h4" style = "right: 47.125%; position: absolute;" bottom = ${id - 5}>我的商店</h6>`
			)
				...
				var i;
					for (i = 0; i < cb.length; i++) { 
						...
						$('#cart').append(
							...
							<h6>$${cb[i].price} /個</h4>
							<h6>庫存: ${cb[i].count}</h4>
							<input type="text" id = ${cb[i].item} count = ${cb[i].count} class = "textareas" placeholder = "多少錢?"></textarea>
							</div>`
								);
							};
							...
							$('#cart').append(
							`
							<button class = "button" id = "buybutton" style = "position: absolute; right: 15px; top: 5px;">採買</button>
