...
MySQL.ready(function()
	MySQL.Async.fetchAll('SELECT name, label FROM vehicle_categories', {}, function(_categories)
		categories = _categories

		MySQL.Async.fetchAll('SELECT category, categoryLabel FROM vehicles', {}, function(_vehicles)
			vehicles = _vehicles
...
RegisterNetEvent('esx_vehicleshop2:setVehicleOwnedPlayerId')
AddEventHandler('esx_vehicleshop2:setVehicleOwnedPlayerId', function(playerId, vehicleProps, model, label)
				...
				MySQL.Async.execute('DELETE FROM cardealer_vehicles WHERE id = @id', {
					...
					if rowsChanged == 1 then
						MySQL.Async.execute('INSERT INTO owned_vehicles (owner, plate, vehicle) VALUES (@owner, @plate, @vehicle)', {
							...
							TriggerClientEvent('esx:showNotification', xPlayer.source, _U('vehicle_set_owned', vehicleProps.plate, xTarget.getName()))
...
RegisterNetEvent('esx_vehicleshop2:rentVehicle')
AddEventHandler('esx_vehicleshop2:rentVehicle', function(vehicle, plate, rentPrice, playerId)
	local xPlayer, xTarget = ESX.GetPlayerFromId(source), ESX.GetPlayerFromId(playerId)
				...
				MySQL.Async.execute('DELETE FROM cardealer_vehicles WHERE id = @id', {
					...
					if rowsChanged == 1 then
						MySQL.Async.execute('INSERT INTO rented_vehicles (vehicle, plate, player_name, base_price, rent_price, owner) VALUES (@vehicle, @plate, @player_name, @base_price, @rent_price, @owner)', {
							...
							TriggerClientEvent('esx:showNotification', xPlayer.source, _U('vehicle_set_rented', plate, xTarget.getName()))
						end)
					end
				end)
			end
...
RegisterNetEvent('esx_vehicleshop2:getStockItem')
AddEventHandler('esx_vehicleshop2:getStockItem', function(itemName, count)
	...
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_cardealer', function(inventory)
		...
		if count > 0 and item.count >= count then
			...
			if sourceItem.count + count <= sourceItem.limit or sourceItem.limit == -1 then
				inventory.removeItem(itemName, count)
				xPlayer.addInventoryItem(itemName, count)
				TriggerEvent('azk_discordlogs:CardealLogs', 'get', xPlayer.source, item.label, count)
...
RegisterNetEvent('esx_vehicleshop2:putStockItems')
AddEventHandler('esx_vehicleshop2:putStockItems', function(itemName, count)
	...
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_cardealer', function(inventory)
		...
		if item.count >= 0 then
			if count <= sourceItem.count then
				xPlayer.removeInventoryItem(itemName, count)
				inventory.addItem(itemName, count)
				TriggerEvent('azk_discordlogs:CardealLogs', 'put', xPlayer.source, item.label, count)
...
ESX.RegisterServerCallback('esx_vehicleshop2:buyVehicle', function(source, cb, model, plate)
	...
	if modelPrice and xPlayer.getMoney() >= modelPrice then
		...
		MySQL.Async.execute('INSERT INTO owned_vehicles (owner, plate, model, price, vehicle) VALUES (@owner, @plate, @model, @price, @vehicle)', {
			...
			TriggerEvent("azk_discordlogs:BuyVehicle", xPlayer.source, model, plate)
			cb(true)
		end)
	else
		cb(false)
...
ESX.RegisterServerCallback('esx_vehicleshop2:buyCarDealerVehicle', function(source, cb, model)
	local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterNetEvent('esx_vehicleshop2:returnProvider')
				...
				MySQL.Async.execute('DELETE FROM cardealer_vehicles WHERE id = @id', {
					...
					if rowsChanged == 1 then
						TriggerEvent('esx_addonaccount:getSharedAccount', 'society_cardealer', function(account)
							...
							TriggerClientEvent('esx:showNotification', xPlayer.source, _U('vehicle_sold_for', vehicleLabel, ESX.Math.GroupDigits(price)))
						end)
					end
				end)
			else
...
RegisterNetEvent('esx_vehicleshop2:setJobVehicleState')
			...
			table.insert(tasks, function(cb)
				...
				if xPlayer then
					if xPlayer.getAccount('bank').money >= v.rent_price then
						...
						TriggerClientEvent('esx:showNotification', xPlayer.source, _U('paid_rental', ESX.Math.GroupDigits(v.rent_price), v.plate))
					else
						TriggerClientEvent('esx:showNotification', xPlayer.source, _U('paid_rental_evicted', ESX.Math.GroupDigits(v.rent_price), v.plate))
