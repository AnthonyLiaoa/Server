...
function OpenShopMenu(zone)
  ...
  ESX.UI.Menu.Open(
    'default', GetCurrentResourceName(), '軍械購物區',
    {
      title  = _U('shop'),
      elements = elements
    },
...
AddEventHandler('esx_weashop:clipcli', function()
  ...
  if IsPedArmed(ped, 4) then
    ...
    if hash~=nil then
      ...
      exports['mythic_notify']:SendAlert('success', '你用過裝載機')
    else
      exports['mythic_notify']:SendAlert('error', '你手上沒有武器')
    end
  else
    exports['mythic_notify']:SendAlert('error', '這種類型的通訊不適合')
  end
end)