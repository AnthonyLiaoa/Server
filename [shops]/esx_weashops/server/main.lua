  ...
  if zone=="BlackWeashop" then
    if account.money >= price then
		if itemName == "clip" then
			...
			TriggerClientEvent('esx:showNotification', xPlayer.source, _U('buy') .. "chargeur")
		else
			xPlayer.addWeapon(itemName, 42)
			TriggerClientEvent('esx:showNotification', xPlayer.source, _U('buy') .. ESX.GetWeaponLabel(itemName))
		end
		xPlayer.removeAccountMoney('black_money', price)
	else
		TriggerClientEvent('esx:showNotification', xPlayer.source, _U('not_enough_black'))
	end
		...
		if itemName == "clip" then
			...
			xPlayer.addWeapon(itemName, 42)
			TriggerClientEvent('esx:showNotification', xPlayer.source, _U('buy') .. ESX.GetWeaponLabel(itemName))
      TriggerEvent('esx:BuyWeaponDraBot', xPlayer.name, ESX.GetWeaponLabel(itemName), '', price, '武器')
		end
