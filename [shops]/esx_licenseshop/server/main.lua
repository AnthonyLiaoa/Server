...
-- Imports License
RegisterServerEvent('esx_licenseshop:buyLicenseImports')
AddEventHandler('esx_licenseshop:buyLicenseImports', function()
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(source)

	if xPlayer.get('money') >= Config.ImportsLicensePrice then
		xPlayer.removeMoney(Config.ImportsLicensePrice)

		TriggerEvent('esx_license:addLicense', _source, 'drive_importedcar', function()
			LoadLicenses(_source)
		end)
	else
		TriggerClientEvent('esx:showNotification', _source, _U('not_enough_money'))
