...
function OpenLicenseShop()
	...
	if ownedLicenses['dmv'] then
		...
		if not ownedLicenses['drive_importedcar'] then
			table.insert(elements, {label = _U('license_imports') .. ' <span style="color: green;">$' .. Config.ImportsLicensePrice .. '</span>', value = 'buy_license_imports'})
		end
	end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'license_shop_actions', {
		...
		elseif data.current.value == 'buy_license_weed' then
			...
			menu.close()
		elseif data.current.value == 'buy_license_imports' then
			TriggerServerEvent('esx_licenseshop:buyLicenseImports')
			menu.close()
		end
