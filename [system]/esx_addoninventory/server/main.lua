...
MySQL.ready(function()
	...
	local result = MySQL.Sync.fetchAll('SELECT name, label, shared FROM addon_inventory')
	...
	for i=1, #result, 1 do
		...
		local result2 = MySQL.Sync.fetchAll('SELECT name, count, owner FROM addon_inventory_items WHERE inventory_name = @inventory_name', {
			['@inventory_name'] = name
		})
