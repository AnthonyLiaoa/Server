...
MySQL.ready(function()
	local result = MySQL.Sync.fetchAll('SELECT name, label, shared FROM datastore')
	...
	for i=1, #result, 1 do
		...
		local result2 = MySQL.Sync.fetchAll('SELECT name, owner, data FROM datastore_data WHERE name = @name', {
			['@name'] = name
		})
