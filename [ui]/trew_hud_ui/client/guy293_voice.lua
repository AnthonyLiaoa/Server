...
local voice = {default = 10.0, shout = 50.0, whisper = 2.0, current = 0, level = nil}
...
function drawPid(r, g, b, a, pid)
    ...
    AddTextComponentString("~s~ID:~r~ " .. pid)
    EndTextCommandDisplayText(0.285, 0.92)
end
...
function drawLevel(r, g, b, a)
	...
	if IsPedInAnyPoliceVehicle(GetPlayerPed(-1)) and GetPedInVehicleSeat(GetVehiclePedIsIn(GetPlayerPed(-1)), -1) == GetPlayerPed(-1) then
		AddTextComponentSubstringPlayerName('~y~語音: ~b~' .. voicepolice.level)
	elseif IsPedInAnyVehicle(GetPlayerPed(-1), false) then
		AddTextComponentSubstringPlayerName('~y~語音: ~b~' .. voicecar.level)
	else
		AddTextComponentSubstringPlayerName('~y~語音: ~b~' .. voice.level)
	end
	EndTextCommandDisplayText(0.285, 0.94)
end
