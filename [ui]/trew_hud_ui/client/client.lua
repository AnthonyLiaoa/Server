...
Citizen.CreateThread(function()
	...
	TriggerEvent('esx:getSharedObject', function(obj)
  	...
  	if ESX.PlayerData.job ~= nil then
        ...
        -- end
		local job
		if MyJob and ESX.PlayerData.job.name == 'unemployed' then
			job = MyJob.label
		elseif MyJob then
			job = ('%s: %s [%s]'):format(ESX.PlayerData.job.label, ESX.PlayerData.job.grade_label, MyJob.label)
		else
			job = ('%s: %s'):format(ESX.PlayerData.job.label, ESX.PlayerData.job.grade_label)
		end
  	...
  	SendNUIMessage({ action = 'setMoney', id = 'blackMoney', value = blackMoney })
		...
		elseif ESX.PlayerData.job.name == 'rebel' and ESX.PlayerData.job.grade ~= 0 then
			if (Config.ui.showSocietyMoney == true) then
				SendNUIMessage({ action = 'element', task = 'enable', value = 'society' })
			end
			ESX.TriggerServerCallback('esx_society:getSocietyMoney', function(money)
