...
local healthActive, armorActive, hungerActive, thirstActive, staminaActive, oxygenActive, microphoneActive, timeActive, cinematicActive, idActive, jobActive, timedivActive, walletdivActive, bankdivActive, societydivActive, blackMoneydivActive, main2Active, mainActive, logoActive
local healthSwitch, armorSwitch, hungerSwitch, thirstSwitch, staminaSwitch, oxygenSwitch, microphoneSwitch, timeSwitch, cinematicSwitch, idSwitch, jobSwitch, timedivSwitch, walletdivSwitch, bankdivSwitch, societydivSwitch, blackMoneydivSwitch, main2Switch, mainSwitch, logoSwitch
...
jobActive = true
jobSwitch = true
SendNUIMessage({action = 'job2Hide'})
else
jobActive = false
jobSwitch = false
SendNUIMessage({action = 'job2Show'})
end
...
RegisterKeyMapping('hud', 'UI設定選單', 'keyboard', Config.openKey)
...
CreateThread(function()
  while 1 do
    TriggerEvent('esx:getSharedObject', function(ESX)
      if ESX.IsPlayerLoaded() and ESX.PlayerData and ESX.PlayerData.job then
...
-- RegisterNetEvent('oc_hud:setSociety')
-- AddEventHandler('oc_hud:setSociety', function(v)
--   society = v
-- end)
-- Money
...
Citizen.CreateThread(function() 
  while true do
    ...
    TriggerEvent('esx:getSharedObject', function(ESX)
      local bank, black = 0, 0
      for k, v in pairs(ESX.PlayerData.accounts) do
        if v.name == 'bank' then bank = v.money
        elseif v.name == 'black_money' then black = v.money
        end
      end
      SendNUIMessage({
        wallet = ESX.PlayerData.money,
        bank = bank,
        black_money = black,
        society = society
      })
...
AddEventHandler("oc_hud:setValues", function(wallet, bank, black_money, society)
...
--local MyJob
RegisterNetEvent('mk_jobs:setJob')
AddEventHandler('mk_jobs:setJob', function(job)
	MyJob = job
end)
...
Citizen.Wait(1000)
if MyJob then
local job2Name = MyJob.label
if(lastJob2 ~= job2Name) then
lastJob2 = job2Name
SendNUIMessage({
action = 'setJob2',
data = job2Name
...
RegisterNetEvent('oc_hud:update')
AddEventHandler('oc_hud:update', function(col, val)
  SendNUIMessage({
    action = 'update',
    col = col,
    val = val
