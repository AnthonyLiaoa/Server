...
$("#job2").draggable({
  drag: function (event, ui) {
    dragpositionjobTop = ui.position.top;
    dragpositionjobLeft = ui.position.left;
    localStorage.setItem("job2Top", dragpositionjobTop);
    localStorage.setItem("job2Left", dragpositionjobLeft);
  }
});
$("#logo").draggable({
  drag: function (event, ui) {
...
window.addEventListener("message", function (event) {
  switch (event.data.action) {
    ...
    case "setPosition":
      ...
      $("#job2").animate({ top: localStorage.getItem("job2Top"), left: localStorage.getItem("job2Left") });
      ...
      $("#darkZonediv").animate({ top: localStorage.getItem("darkZonedivTop"), left: localStorage.getItem("darkZonedivLeft") });
    ...
    case "hud":
      ...
      $("#societydiv").form(event.data.societydiv);
      ...
      $("#logo").form(event.data.logo);
      $("#main").form(event.data.main);
      $("#main2").form(event.data.main2);
      $("#voice").form(event.data.voice);
    ...
    case "logoShow":
      ...
      break;
    case 'update':
      $("#"+event.data.col).html(event.data.val);
      break;
  }
});
...
$("#job2-switch").click(function () { $.post('https://oc_hud/change', JSON.stringify({ action: 'job2' })) });
...
$("#societydiv-switch").click(function () { $.post('https://oc_hud/change', JSON.stringify({ action: 'societydiv' })) });
$("#blackMoneydiv-switch").click(function () { $.post('https://oc_hud/change', JSON.stringify({ action: 'blackMoneydiv' })) });
$("#logo-switch").click(function () { $.post('https://oc_hud/change', JSON.stringify({ action: 'logo' })) });
...
$("#reset-position").click(function () {
  ...
  $("#job2").animate({ top: "0px", left: "0px" });
  localStorage.setItem("job2Top", "0px");
  localStorage.setItem("job2Left", "0px");
...
$(function () {
  window.addEventListener('message', function (event) {
    switch (event.data.action) {
      case 'setJob2':
        $('.job2-text').text(event.data.data)
        break
