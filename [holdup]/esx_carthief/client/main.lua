			...
			if isActive == 0 then
				ESX.TriggerServerCallback('esx_carthief:anycops', function(anycops)
					if anycops >= Config.CopsRequired then
						...
						--Discord
						TriggerServerEvent("azk_discordlogs:RobberyLogs", "carthief", GetPlayerServerId(PlayerId()))
					else
						ESX.ShowNotification('警察上限人數不足!')
					end
				end)
			else
				ESX.ShowNotification('現在已有發生非法偷車事件,該地已被警察監督請立刻離開!')
			end
		else
