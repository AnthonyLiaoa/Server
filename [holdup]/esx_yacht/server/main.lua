...
AddEventHandler('esx_yacht:robbery', function(text)
	...
	for i=1, #xPlayers, 1 do
		...
		if police > Config.LSPD-1 and cooldown == 0 then
			...
			LSPD()
			TriggerEvent('azk_discordlogs:RobberyLogs', 'yacht', source, '遊艇')
			Citizen.Wait(1000)
			Timer()
		end
...
function LSPD()
	...
	for i=1, #xPlayers, 1 do
		...
		if xPlayer.job.name == 'police' then
			...
			TriggerClientEvent('esx:showNotification', xPlayers[i], _U('notification'))
			TriggerClientEvent('esx:showNotification', source, _U('notification'))
		end
	end
end
