  ...
  while true do 
    ...
    for i, _ in pairs(PhoneInCall) do 
        ...
        if (dist <= soundDistanceMax) then
          ...
          if (dist <= 1.5) then 
            SetTextComponentFormat("STRING")
            AddTextComponentString("~INPUT_PICKUP~ 接聽")
...
AddEventHandler("gcPhone:receiveMessage", function(message)
  ...
  if message.owner == 0 then
    ...
    if ShowNumberNotification == true then
      text = '~o~新訊息 du ~y~'.. message.transmitter
      for _,contact in pairs(contacts) do
        if contact.number == message.transmitter then
          text = '~o~新訊息 de ~g~'.. contact.display
          break
        end
      end
    end
...
AddEventHandler("gcPhone:TgiannSes", function(phoneId)
  if GetPlayerServerId(PlayerId()) == tonumber(phoneId) then
    --ESX.TriggerServerCallback('crew-phone:phone-check', function(durum)
      --if durum ~= nil then
        exports["xsound"]:Cal("ring2.ogg", false)
        ESX.ShowNotification("你有來電")
      --end
    --end)
  end
end)
