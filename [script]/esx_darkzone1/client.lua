-- 定義自己的第二暗區(下面輸入的xyz是中心點)
ESX = nil
local darkzone = {
    { ['x'] = 2017.39, ['y'] = 2785.46, ['z'] = 56.63},
}
local notifin = false
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if dist <= 350.0 and IsEntityDead(playerPed) and no == false then
			...
			ESX.ShowAdvancedNotification('第二暗區', '~r~醫療組', '~p~即將在5秒內復活', 'CHAR_BLANK_ENTRY', 7, true, true, 150)
		end
		...
		if dist <= 350.0 and IsEntityDead(playerPed) and no == false then
			...
			if spawnpoint == 1 then
				SetEntityCoords(playerPed, 1989.34, 2841.77, 49.19, true, true, true, false)
				print('a')
				TriggerServerEvent('esx_darkzone:dead1')
			elseif spawnpoint == 2 then
				SetEntityCoords(playerPed, 1979.54, 2801.18, 49.19, true, true, true, false)
				print('b')
				TriggerServerEvent('esx_darkzone:dead2')
			elseif spawnpoint == 3 then
				SetEntityCoords(playerPed, 1987.80, 2745.01, 48.00, true, true, true, false)
				print('c')
				TriggerServerEvent('esx_darkzone:dead3')
			elseif spawnpoint == 4 then
				SetEntityCoords(playerPed, 2016.10, 2707.69, 48.98, true, true, true, false)
				print('d')
				TriggerServerEvent('esx_darkzone:dead4')
			elseif spawnpoint == 5 then
				SetEntityCoords(playerPed, 2048.34, 2733.38, 49.32, true, true, true, false)
				print('e')
				TriggerServerEvent('esx_darkzone:dead5')
			elseif spawnpoint == 6 then
				SetEntityCoords(playerPed, 2058.20, 2759.44, 49.28, true, true, true, false)
				print('f')
				TriggerServerEvent('esx_darkzone:dead6')
			elseif spawnpoint == 7 then
				SetEntityCoords(playerPed, 2050.12, 2801.75, 49.29, true, true, true, false)
				print('g')
				TriggerServerEvent('esx_darkzone:dead7')
			elseif spawnpoint == 8 then
				SetEntityCoords(playerPed, 2037.34, 2826.2, 49.37, true, true, true, false)
				print('h')
		...
		if dist <= 100.0 then -- 範圍
			if not notifIn and no == false then
				ClearPlayerWantedLevel(PlayerId())
				TriggerEvent("pNotify:SendNotification",{
					text = "<b style='color:#CE0000'>你已經進入第二暗區,請你注意安全</b>",
					type = "warning",
					theme = "metroui",
					timeout = (3000),
					layout = "bottomcenter",
			...
			if not notifOut and no == false then
				TriggerEvent("pNotify:SendNotification",{
					text = "<b style='color:#1E90FF'>你已經離開第二暗區,請你注意安全!</b>",
					type = "error",
					timeout = (3000),
					layout = "bottomcenter",
					queue = "global"
