...
Citizen.CreateThread(function()
  ...
  while true do
    Citizen.Wait(5)
	if (IsControlJustPressed(0, 303)) and hasPressed == false then
		hasPressed = true
		...
		notowned = 0
		if #cars == 0 then
			ESX.ShowNotification("你沒有這輛車的控制權")
		else
			for j=1, #cars, 1 do
				...
				ESX.TriggerServerCallback('carlock:isVehicleOwner', function(owner)
					if owner and hasAlreadyLocked ~= true then
						...
						if lock == 1 or lock == 0 then
							...
							ESX.ShowNotification('車門已經 ~r~上鎖~s~ ')
						...
						elseif lock == 2 then
							...
							ESX.ShowNotification('車門已經 ~g~解鎖~s~ ')
							if not IsPedInAnyVehicle(PlayerPedId(), true) then
								TaskPlayAnim(PlayerPedId(), dict, "fob_click_fp", 8.0, 8.0, -1, 48, 1, false, false, false)
							end
							SetVehicleLights(carstrie[i], 2)
					...
					if notowned == #carstrie then
						ESX.ShowNotification("你沒有這輛車的保安控制權")
					end	
				end, plate)
			end			
		end
