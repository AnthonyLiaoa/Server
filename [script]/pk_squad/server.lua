...
AddEventHandler('playerConnecting', function(name, setCallback)
  	...
  	if(string.sub(identifiers[1], 1, 5) ~= "steam") then
		setCallback("Steam must be running in order to join this server.")
		CancelEvent()
	end
end)
--  Script By Caruby  --
...
Config.Language = {
    ...
    banned = "You are banned, you may appeal it | Reason: %s",
    steam = "Error: Steam must be running",
    prio = "You must be whitelisted to join this server. You may apply"
}
-----------------------------------------------------------------------------------------------------------------------
