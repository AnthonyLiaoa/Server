...
function getVehData(plate, callback)
    MySQL.Async.fetchAll("SELECT vehicle, owner FROM `owned_vehicles`", {},
    function(result)
        ...
        if foundIdentifier ~= nil then
            MySQL.Async.fetchAll("SELECT firstname, lastname FROM `users` WHERE `identifier` = @identifier", {['@identifier'] = foundIdentifier},
            function(result)
