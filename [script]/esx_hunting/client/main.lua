...
local Positions = {
	['StartHunting'] = { ['hint'] = '[E] 開始狩獵', ['x'] = -769.23773193359, ['y'] = 5595.6215820313, ['z'] = 33.48571395874 },
	['Sell'] = { ['hint'] = '[E] 販售獵物', ['x'] = 969.16375732422, ['y'] = -2107.9033203125, ['z'] = 31.475671768188 },
...
function StartHuntingSession()
	...
	if OnGoingHuntSession then
		...
		--HuntCar = CreateVehicle(GetHashKey('sanchez2'), Positions['SpawnATV'].x, Positions['SpawnATV'].y, Positions['SpawnATV'].z, 169.79, true, false)
					...
					if DoesEntityExist(value.id) then
						...
						if AnimalHealth <= 0 then
							...
							if PlyToAnimal < 2.0 then
								...
								ESX.Game.Utils.DrawText3D({x = AnimalCoords.x, y = AnimalCoords.y, z = AnimalCoords.z + 1}, '[E] 屠宰動物', 0.4)
...
function SlaughterAnimal(AnimalId)
	...
	ESX.ShowNotification('您宰殺了動物並收到了 ' ..AnimalWeight.. 'kg')
