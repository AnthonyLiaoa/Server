...
Citizen.CreateThread(function()
	while true do
		...
		if isInMarker and isInZone then
			TriggerEvent('esx_bitcoin:hasEnteredMarker', 'exitMarker')
		end
		...
		if CurrentAction ~= nil then
			...
			if IsControlJustReleased(0, Keys['E']) and IsPedOnFoot(PlayerPedId()) then
				...
				if CurrentAction == 'exitMarker' then
					isInZone = false
					TriggerEvent('esx_bitcoin:freezePlayer', false)
					TriggerEvent('esx_bitcoin:hasExitedMarker', lastZone)
					Citizen.Wait(15000)
				elseif CurrentAction == 'BitCoin' then
					Wait(500)
				...
				if isInZone then 
					TriggerEvent('esx_bitcoin:freezePlayer',true)
				end
...
RegisterNetEvent('esx_bitcoin:freezePlayer')
AddEventHandler('esx_bitcoin:freezePlayer', function(freeze)
