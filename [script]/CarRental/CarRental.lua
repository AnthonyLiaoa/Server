...
Citizen.CreateThread(function()
	...
	pickupStation = { --Set the car rental locaitons here
		{x = -1045.34, y = -2724.29, z = 19.05}, --機場
	}
	
	dropoffStation = { --Set the car dropoff locations here
		{x = -62.77, y = -1083.94, z = 25.93}, --車商
	}	
	...
	WarMenu.CreateMenu('carRental', '車輛出租')
	WarMenu.CreateSubMenu('closeMenu', 'carRental', '你確定嗎？')
	WarMenu.CreateSubMenu('carPicker', 'carRental', '出租車 | 一天等於 ' .. rentalTimer ..  ' 分鐘')
	WarMenu.CreateSubMenu('carInsurance', 'carRental', '是否購買車輛保險')
	WarMenu.CreateMenu('carReturn', '車輛出租')
	WarMenu.SetSubTitle('carReturn', '是否歸還出租車？') 
	WarMenu.CreateMenu('arrestCheck', '車輛歸還')
	WarMenu.SetSubTitle('arrestCheck', '你目前被通緝嗎？')
	
	while true do
		--Main menu
		if WarMenu.IsMenuOpened('carRental') then
			if WarMenu.MenuButton('租車', 'carPicker') then
			elseif WarMenu.MenuButton('車輛保險', 'carInsurance') then
			...
			WarMenu.SetSubTitle('carRental', '收費出租')
			
			WarMenu.Display()
			
		--Close menu
		elseif WarMenu.IsMenuOpened('closeMenu') then
			if WarMenu.Button('是') then
				WarMenu.CloseMenu()
			elseif WarMenu.MenuButton('否', 'carRental') then
			end
		...
		elseif WarMenu.IsMenuOpened('carPicker') then
			if WarMenu.Button('gogoro2|租金/次: $100|租金/日: $50') then
				SpawnVehicle("gogoro2")
				TriggerServerEvent("chargePlayer", 100)
				ESX.ShowNotification("已收取租金$100")
				autoChargeAmount = 50
				isBeingCharged = true
				WarMenu.CloseMenu()
			 elseif WarMenu.Button('啞劇|租金/次: $200|租金/日: $100') then
			 	SpawnVehicle("panto")
			 	TriggerServerEvent("chargePlayer", 200)
			 	ESX.ShowNotification("已收取租金$200")
			 	autoChargeAmount = 100
			 	isBeingCharged = true
			 	WarMenu.CloseMenu()
			-- elseif WarMenu.Button('亞班尼 初代|租金/次: $100|租金/日: $100') then
			-- 	SpawnVehicle("primo")
			-- 	TriggerServerEvent("chargePlayer", 100)
			-- 	ESX.ShowNotification("已收取租金$100")
			-- 	autoChargeAmount = 100
			-- 	isBeingCharged = true
			-- 	WarMenu.CloseMenu()
			-- elseif WarMenu.Button('卡林 入侵者|租金/次: $250|租金/日: $100') then
			-- 	SpawnVehicle("intruder")
			-- 	TriggerServerEvent("chargePlayer", 250)
			-- 	ESX.ShowNotification("已收取租金$250")
			-- 	autoChargeAmount = 100
			-- 	isBeingCharged = true
			-- 	WarMenu.CloseMenu()
			-- elseif WarMenu.Button('冒險家 女巫|租金/次: $1000|租金/日: $300') then
			-- 	SpawnVehicle("banshee")
			-- 	TriggerServerEvent("chargePlayer", 1000)
			-- 	ESX.ShowNotification("已收取租金$1000")
			-- 	autoChargeAmount = 300
			-- 	isBeingCharged = true
			-- 	WarMenu.CloseMenu()
			elseif WarMenu.MenuButton('返回', 'carRental') then
			end
		...
		elseif WarMenu.IsMenuOpened('carInsurance') then
			if WarMenu.Button('是|收費$150') then
...
function returnVehicle()
			...
			damageInsurance = false
			damageCharge = false
			ESX.ShowNotification("感謝您的歸還，歡迎再來！")
			local currentVehicle = GetVehiclePedIsIn(GetPlayerPed(-1), false)
			SetEntityAsMissionEntity(currentVehicle, true, true)
			local x, y, z = table.unpack(GetEntityCoords(GetPlayerPed(-1), false))
