...
TriggerEvent('chat:addSuggestion', '/givecar', 'Give a vehicle to player', {
	...
	{ name="type", help="Vehicle type, car, boat or aircraft" },
...
AddEventHandler('esx_giveownedcar:spawnVehicle', function(playerID, model, cartype, playerName, type)
	...
	ESX.Game.SpawnVehicle(model, coords, 0.0, function(vehicle) --get vehicle info
		if DoesEntityExist(vehicle) then
			...
			TriggerServerEvent('esx_giveownedcar:setVehicle', vehicleProps, cartype, playerID)
			ESX.Game.DeleteVehicle(vehicle)	
			if type ~= 'console' then
				ESX.ShowNotification(_U('gived_car', model, newPlate, playerName))
			else
				local msg = ('addCar: ' ..model.. ', Vehicle type: '..cartype..', plate: ' ..newPlate.. ', toPlayer: ' ..playerName)
...
AddEventHandler('esx_giveownedcar:spawnVehiclePlate', function(playerID, model, cartype, plate, playerName, type)
	local playerPed = GetPlayerPed(-1)
	local coords    = GetEntityCoords(playerPed)
