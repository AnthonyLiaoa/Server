...
function CreateScale(sType)
	if scaleString ~= sType and sType == "OpenShop" then
		scaleType = setupScaleform("instructional_buttons", "開始刺青", 38)
		scaleString = sType
	elseif scaleString ~= sType and sType == "Control" then
		scaleType = setupScaleform2("instructional_buttons", "更改視角", 108, "透明度", {90, 89}, "購買刺青", 191)
		scaleString = sType
	end
end
