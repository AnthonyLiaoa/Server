	...
	if not playersProcessingMeth[source] then
		...
		if GetCops() >= Config.RequiredCops then
			playersProcessingMeth[_source] = ESX.SetTimeout(Config.Delays.MethProcessing, function()
				...
				if xmeth.limit ~= -1 and (xmeth.count + 2) > xmeth.limit then
					TriggerClientEvent('esx:showNotification', _source, _U('meth_processingfull'))
				elseif xhydrochloric_acid.count < 6 then
					TriggerClientEvent('esx:showNotification', _source, _U('meth_processingenough'))
				elseif xsulfuric_acid.count < 6 then
					TriggerClientEvent('esx:showNotification', _source, _U('meth_processingenough'))
				elseif xsodium_hydroxide.count < 6 then
					TriggerClientEvent('esx:showNotification', _source, _U('meth_processingenough'))
				else
					xPlayer.removeInventoryItem('hydrochloric_acid', 6)
					xPlayer.removeInventoryItem('sulfuric_acid', 6)
					xPlayer.removeInventoryItem('sodium_hydroxide', 6)
