...
AddEventHandler('esx:playerLoaded', function(xPlayer)
	PlayerData = xPlayer
	TriggerServerCallback('esx_lscustom:getVehiclesPrices', function(vehicles)
		Vehicles = vehicles
	end)
end)
			...
			if k == data.current.modType or isRimMod then
				...
				if data.current.label == _U('by_default') or string.match(data.current.label, _U('installed')) then
					...
					elseif v.modType == 11 or v.modType == 12 or v.modType == 13 or v.modType == 15 or v.modType == 16 then
						price = math.floor(vehiclePrice * v.price[data.current.modNum + 1] / 10)
						TriggerServerEvent("esx_lscustom:buyMod", price)
					elseif v.modType == 17 then
						price = math.floor(vehiclePrice * v.price[1] / 10)
						TriggerServerEvent("esx_lscustom:buyMod", price)
					else
						price = math.floor(vehiclePrice * v.price / 10)
						TriggerServerEvent("esx_lscustom:buyMod", price)
					end
				end
		...
		if data.value == k then
			...
			if v.modType ~= nil then
				...
				elseif v.modType == 11 or v.modType == 12 or v.modType == 13 or v.modType == 15 or v.modType == 16 then
					...
					for j = 0, modCount, 1 do
						...
						if j == currentMods[k] then
							...
							price = math.floor(vehiclePrice * v.price[j+1] / 10)
				...
				elseif v.modType == 17 then -- TURBO
					...
					if ESX.GetPlayerData().job.grade >= 6 then
						table.insert(elements, {label = _label, modType = k, modNum = true})
					end
				else
