    ...
    function (data, menu)
      ...
      ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'shop_confirm',
        {
          ...
          align = 'top-left',
          elements = {
            {label = '' .. vehicleData.name .. _U('costs') .. _U('currency') .. vehicleData.price * Config.Price, value = 'yes'},
            {label = _U('back'), value = 'no'},
          },
        },
        function (data2, menu2)
          if data2.current.value == 'yes' then
             --sendNotification(_U('contact_dealer')  .. _U('currency') .. vehicleData.price * Config.Price, 'warning', 5000)
          end
...
function WaitForVehicleToLoad(modelHash)
	...
	if not HasModelLoaded(modelHash) then
		...
		sendNotification(_U('shop_awaiting_model'))
