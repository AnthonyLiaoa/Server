...
function InJail()
	...
	Citizen.CreateThread(function()
		...
		while jailTime > 0 do
			...
			ESX.ShowNotification("你還有" .. jailTime .. "個月的刑期")
					...
					if DistanceCheck <= 1.5 then
						...
						if IsControlJustPressed(0, 38) then
							...
							if v["state"] then
								...
								ESX.ShowNotification("你已經拿取了這個包裹!")
							end
		...
		while true do
			...
			for p, v in pairs(Config.Teleports) do
				...
				if DistanceCheck <= 7.5 then
					...
					ESX.Game.Utils.DrawText3D(v, "按[E]開門", 0.4)
...
function PackPackage(packageId)
	...
	while Packaging do
		...
		if PackPercent >= 100 then
			...
			ESX.Game.Utils.DrawText3D(Package, "已打包:" .. math.ceil(tonumber(PackPercent)) .. "%", 0.4)
		end
...
function DeliverPackage(packageId)
	...
	while Packaging do
		...
		if not IsEntityAttachedToEntity(packageId, PlayerPedId()) then
			...
			ESX.Game.Utils.DrawText3D(DeliverPosition, "按[E]放下包裹", 0.4)
...
function OpenJailMenu()
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'jail_prison_menu',
		{
			...
			align    = 'center',
			elements = {
				{ label = "監獄玩家", value = "jail_closest_player" },
				{ label = "釋放玩家", value = "unjail_player" }
			}
		}, 
	function(data, menu)
						...
						function(data3, menu3)
						  	...
						  	if reason == nil then
								...
								if closestPlayer == -1 or closestDistance > 3.0 then
									...
									TriggerServerEvent("esx:tojail", jailTime, reason)  
								end
		...
		elseif action == "unjail_player" then
			...
			ESX.TriggerServerCallback("esx-qalle-jail:retrieveJailedPlayers", function(playerArray)
				...
				if #playerArray == 0 then
					ESX.ShowNotification("監獄裡無人")
					return
				end

				for i = 1, #playerArray, 1 do
					table.insert(elements, {label = "囚犯: " .. playerArray[i].name .. " | 剩餘時間: " .. playerArray[i].jailTime .. " 分鐘", value = playerArray[i].identifier })
				end
