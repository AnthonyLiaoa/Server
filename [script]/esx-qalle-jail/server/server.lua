	...
	if xPlayer["job"]["name"] == "police" then
		...
		if GetPlayerName(jailPlayer) ~= nil then
			...
			if jailTime ~= nil then
				...
				TriggerClientEvent("esx:showNotification", src, GetPlayerName(jailPlayer) .. " 被關了 " .. jailTime .. " 分鐘")
				
				if args[3] ~= nil then
					GetRPName(jailPlayer, function(Firstname, Lastname)
						TriggerClientEvent('chat:addMessage', -1, { args = { "監獄",  Firstname .. " " .. Lastname .. " 被關原因: " .. args[3] }, color = { 249, 166, 0 } })
					end)
				end
			else
				TriggerClientEvent("esx:showNotification", src, "時間錯誤")
			end
		else
			TriggerClientEvent("esx:showNotification", src, "此ID不在線上")
		end
	else
		TriggerClientEvent("esx:showNotification", src, "你不是警察")
	end
end)
...
AddEventHandler("esx-qalle-jail:jailPlayer", function(targetSrc, jailTime, jailReason)
	...
	GetRPName(targetSrc, function(Firstname, Lastname)
		TriggerClientEvent('chat:addMessage', -1, { args = { "監獄",  Firstname .. " " .. Lastname .. " 被監禁原因: " .. jailReason }, color = { 249, 166, 0 } })
	end)

	TriggerClientEvent("esx:showNotification", src, GetPlayerName(targetSrc) .. " 被關了 " .. jailTime .. " 分鐘")
end)
...
AddEventHandler("esx-qalle-jail:prisonWorkReward", function()
	...
	TriggerClientEvent("esx:showNotification", src, "感謝,你有錢能飽餐一頓了")
end)
