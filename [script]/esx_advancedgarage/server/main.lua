...
MySQL.ready(function()
	if Config.RestartParkVehicles == true then
		ParkVehicles()
		print("All vehicles returned to the grage")
	end
end)
...
ESX.RegisterServerCallback('esx_advancedgarage:getOwnedAircrafts', function(source, cb)
	...
	if Config.DontShowPoundCarsInGarage == true then
		MySQL.Async.fetchAll('SELECT vehicle, stored, plate FROM owned_vehicles WHERE owner = @owner AND Type = @Type AND job = @job AND `stored` = @stored', {
		...
		MySQL.Async.fetchAll('SELECT vehicle, stored, plate FROM owned_vehicles WHERE owner = @owner AND Type = @Type AND job = @job', {
			['@owner']  = GetPlayerIdentifiers(source)[1],
			['@Type']   = 'aircraft',
			['@job']    = ''
		}, function(data)
...
ESX.RegisterServerCallback('esx_advancedgarage:getOutOwnedAircrafts', function(source, cb)
	...
	MySQL.Async.fetchAll('SELECT vehicle FROM owned_vehicles WHERE owner = @owner AND Type = @Type AND job = @job AND `stored` = @stored', {
		['@owner'] = GetPlayerIdentifiers(source)[1],
		['@Type']   = 'aircraft',
		['@job']    = '',
		['@stored'] = false
...
ESX.RegisterServerCallback('esx_advancedgarage:getOutOwnedJobCars', function(source, cb)
	local ownedJobCars = {}

	MySQL.Async.fetchAll('SELECT vehicle FROM owned_vehicles WHERE owner = @owner AND job = @job AND `stored` = @stored', {
		['@owner'] = GetPlayerIdentifiers(source)[1],
		['@job']    = ESX.GetPlayerFromId(source).job.name,
		['@stored'] = false
	}, function(data) 
		for _,v in pairs(data) do
			local vehicle = json.decode(v.vehicle)
			table.insert(ownedJobCars, vehicle)
		end
		cb(ownedJobCars)
	end)
end)
