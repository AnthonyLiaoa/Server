	...
	ESX.TriggerServerCallback('esx_property:getPropertyInventory', function(inventory)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'room_inventory',
		{
			...
			if data.current.type == 'item_weapon' then
					menu.close()
					ESX.TriggerServerCallback('esx_property:getItem', function()
					ESX.SetTimeout(150, function()
						OpenRoomInventoryMenu(property, owner)
					end)
				end, owner, data.current.type, data.current.value, data.current.ammo, data.current.index)
				...
				ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'get_item_count', {
					...
					if quantity == nil then
						...
						end, owner, data.current.type, data.current.value, quantity)
					end
			...
			if data.current.type == 'item_weapon' then
				if data.current.ammo > 0 then 
					ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
					  title = '子彈數量  (目前身上有 :'..data.current.ammo..' 顆'
					}, function(data2, menu2)
						local count = tonumber(data2.value)
						if count == nil or count > data.current.ammo  then
							ESX.ShowNotification(_U('quantity_invalid'))
						else
							menu2.close()
							menu.close()

							ESX.TriggerServerCallback('esx_property:putItem', function() 
								ESX.SetTimeout(150, function()
									OpenPlayerInventoryMenu(property, owner)
								end)
							end,owner, data.current.type, data.current.value, count)
					end
					end, function(data2, menu2)
					menu2.close()
					end)
					...
					end,owner, data.current.type, data.current.value,  data.current.ammo)
				end
			else
