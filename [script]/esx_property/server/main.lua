...
AddEventHandler('esx_ownedproperty:getOwnedProperties', function(cb)
	MySQL.Async.fetchAll('SELECT id, name, price, rented, owner FROM owned_properties', {}, function(result)
		local properties = {}
...
AddEventHandler('esx_property:rentProperty', function(propertyName)
	...
	local rent     = ESX.Math.Round(property.price / 2)
...
AddEventHandler('esx_property:buyProperty', function(propertyName)
	...
	if property.price <= xPlayer.getMoney() then
		...
		TriggerEvent('azk_discordlogs:PropertyLogs', 'purchased', source, propertyName, property.price)
	else
...
AddEventHandler('esx_property:removeOwnedProperty', function(propertyName)
	...
	if xPlayer then 
		local getBack = math.floor(property.price* 0.6)
		xPlayer.addMoney(getBack)
		TriggerClientEvent('esx:showNotification', source, "你拿回了".. getBack)
	end
end)
...
ESX.RegisterServerCallback('esx_property:getItem', function(source, cb,owner, type, item, count,index)
		...
		TriggerEvent('esx_addoninventory:getInventory', 'property', xPlayerOwner.identifier, function(inventory)
			...
			if count > 0 and inventoryItem.count >= count then
				...
				if sourceItem.limit ~= -1 and (sourceItem.count + count) > sourceItem.limit then
					...
					TriggerEvent('azk_discordlogs:PropertyLogs', 'get', _source, 0, 0, inventoryItem.label, count)
				end
			else
	...
	elseif type == 'item_account' then
		...
		TriggerEvent('esx_addonaccount:getAccount', 'property_' .. item, xPlayerOwner.identifier, function(account)
			...
			if roomAccountMoney >= count then
				...
				TriggerEvent('azk_discordlogs:PropertyLogs', 'get', _source, 0, 0, item, count)
			else
	...
	elseif type == 'item_weapon' then
		...
		TriggerEvent('esx_datastore:getDataStore', 'property', xPlayerOwner.identifier, function(store)
			...
			for i=1, #storeWeapons, 1 do
				if storeWeapons[i].index == nil then
					storeWeapons[i].index = i
				end
				if storeWeapons[i].index == index then
			...
			TriggerEvent('azk_discordlogs:PropertyLogs', 'get', _source, 0, 0, weaponName, ammo)
			cb()
		end)
...
ESX.RegisterServerCallback('esx_property:putItem', function(source, cb,owner, type, item, count)
	local _source      = source
	local xPlayer      = ESX.GetPlayerFromId(_source)
	local xPlayerOwner = ESX.GetPlayerFromIdentifier(owner)
	...
	if type == 'item_standard' then
		...
		if playerItemCount >= count and count > 0 then
			TriggerEvent('esx_addoninventory:getInventory', 'property', xPlayerOwner.identifier, function(inventory)
				...
				TriggerEvent('azk_discordlogs:PropertyLogs', 'put', _source, 0, 0, inventory.getItem(item).label, count)
				cb()
			end)
		else
	...
	elseif type == 'item_account' then
		...
		if playerAccountMoney >= count and count > 0 then
			...
			TriggerEvent('azk_discordlogs:PropertyLogs', 'put', _source, 0, 0, item, count)
		else
			TriggerClientEvent('esx:showNotification', _source, _U('amount_invalid'))
			cb()
		end
	...
	elseif type == 'item_weapon' then
		...
		print("PutItem: ==============\n", ESX.DumpTable(xPlayer.getLoadout()), "==============\n")

		TriggerEvent('esx_datastore:getDataStore', 'property', xPlayerOwner.identifier, function(store)
			local storeWeapons = store.get('weapons') or {}
			if #storeWeapons == 0 then
				table.insert(storeWeapons, {
					index = 1,
					name = item,
					ammo = count,
					components = xPlayer.loadout[loadoutNum].components
				})
			else
				if storeWeapons[#storeWeapons].index == nil then
					for i=1 ,#storeWeapons ,1 do
						storeWeapons[i].index = i
					end
				end
				table.insert(storeWeapons, {
					index = storeWeapons[#storeWeapons].index + 1,
					name = item,
					ammo = count,
					components = xPlayer.loadout[loadoutNum].components
				})
			...
			TriggerEvent('azk_discordlogs:PropertyLogs', 'putwp', _source, 0, 0, item)
			cb()
		end)
...
function PayRent(d, h, m)
	...
	MySQL.Async.fetchAll('SELECT owner,price FROM owned_properties WHERE rented = 1', {}, function (result)
		for k,v in ipairs(result) do
			table.insert(tasks, function(cb)
				...
				if xPlayer then
					...
					TriggerClientEvent('esx:showNotification', xPlayer.source, _U('paid_rent', ESX.Math.GroupDigits(v.price)))
