...
AddEventHandler('fuel:startFuelUpTick', function(pumpObject, ped, vehicle)
	...
	while isFueling do
		...
		local fuelToAdd = math.random(18, 20) / 10.0
		local extraCost = fuelToAdd * Config.FuelPrice
...
AddEventHandler('fuel:refuelFromPump', function(pumpObject, ped, vehicle)
	...
	while isFueling do
		...
		if pumpObject then
			...
			DrawText3Ds(stringCoords.x, stringCoords.y, stringCoords.z + 1.3, Config.Strings.CancelFuelingPump .. extraString)
	...
	while true do
		...
		if not isFueling and ((isNearPump and GetEntityHealth(isNearPump) > 0) or (GetSelectedPedWeapon(ped) == 883325847 and not isNearPump)) then
			if IsPedInAnyVehicle(ped) and GetPedInVehicleSeat(GetVehiclePedIsIn(ped), -1) == ped then
				...
				DrawText3Ds(pumpCoords.x, pumpCoords.y, pumpCoords.z + 1.7, Config.Strings.ExitVehicle)
			else
					...
					if not DoesEntityExist(GetPedInVehicleSeat(vehicle, -1)) then
						...
						if GetVehicleFuelLevel(vehicle) < 95 and canFuel then
							if currentCash > 0 then
								DrawText3Ds(stringCoords.x, stringCoords.y, stringCoords.z + 1.7, Config.Strings.EToRefuel)
								...
								DrawText3Ds(stringCoords.x, stringCoords.y, stringCoords.z + 1.3, Config.Strings.NotEnoughCash)
							end
						elseif not canFuel then
							DrawText3Ds(stringCoords.x, stringCoords.y, stringCoords.z + 1.3, Config.Strings.JerryCanEmpty)
						else
							DrawText3Ds(stringCoords.x, stringCoords.y, stringCoords.z + 1.3, Config.Strings.FullTank)
						end
					end
				elseif isNearPump then
					...
					if currentCash >= Config.JerryCanCost then
						if not HasPedGotWeapon(ped, 883325847) then
							DrawText3Ds(stringCoords.x, stringCoords.y, stringCoords.z + 1.3, Config.Strings.PurchaseJerryCan)
							...
							if Config.UseESX then
								...
								if refillCost > 0 then
									if currentCash >= refillCost then
										DrawText3Ds(stringCoords.x, stringCoords.y, stringCoords.z + 1.3, Config.Strings.RefillJerryCan .. refillCost)
										...
										DrawText3Ds(stringCoords.x, stringCoords.y, stringCoords.z + 1.3, Config.Strings.NotEnoughCashJerryCan)
									end
								else
									DrawText3Ds(stringCoords.x, stringCoords.y, stringCoords.z + 1.3, Config.Strings.JerryCanFull)
								end
							else
								DrawText3Ds(stringCoords.x, stringCoords.y, stringCoords.z + 1.3, Config.Strings.RefillJerryCan)
	...
	Citizen.CreateThread(function()
		while true do
			...
			if displayHud then
				...
				--DrawAdvancedText(0.2375 - x, 0.80- y, 0.005, 0.0028, 0.3, fuel, 255, 215, 0, 255, 6, 1)
				DrawAdvancedText(0.4017, 0.976, 0.005, 0.0028, 0.3, fuel, 255, 215, 0, 255, 6, 1)
				--DrawAdvancedText(0.215 - x, 0.80 - y, 0.005, 0.0028, 0.3, "油量:      %", 255, 255, 255, 255, 6, 1)
				DrawAdvancedText(0.380, 0.976, 0.005, 0.0028, 0.3, "油量:      %", 255, 255, 255, 255, 6, 1)
			else
				Citizen.Wait(750)
			end
		end
