...
RegisterNetEvent("nuo:openInventory")
AddEventHandler("nuo:openInventory", function()
...
RegisterNUICallback("GetNearPlayersPolice", function(data, cb)
    ...
    if not foundPlayers then
        exports.pNotify:SendNotification(
            {
                text = '<strong class="red-text">周圍沒有玩家</strong>',
                type = "error",
                timeout = 3000,
                layout = "bottomCenter",
                queue = "inventoryhud"
...
RegisterNUICallback(
    ...
    function(data, cb)
        ...
        if not foundPlayers then
            exports.pNotify:SendNotification(
                {
                    text = '<strong class="red-text">找不到玩家</strong>',
                    type = "error",
                    timeout = 3000,
                    layout = "bottomCenter",
                    queue = "inventoryhud"
