...
AddEventHandler('esx_inventoryhud:updateKey', function(target, type, itemName)
	...
	if type == "item_key" then -- MEETA GiveKey
		...
		TriggerClientEvent("pNotify:SendNotification", source, {
			text = 'Znalazłeś <strong class="amber-text">Kluczyki od samochodu</strong> Zarejestrój <strong class="yellow-text">'..itemName..'</strong>',
			type = "success",
			timeout = 3000,
			layout = "bottomCenter",
			queue = "global"
		...
		TriggerClientEvent("pNotify:SendNotification", target, {
			text = 'Dostałeś <strong class="amber-text">kluczyki od pojazdu</strong> Musisz zarejetrować <strong class="yellow-text">'..itemName..'</strong>',
			type = "success",
			timeout = 3000,
			layout = "bottomCenter",
			queue = "global"
	...
	elseif type == "item_keyhouse" then -- MEETA GiveKeyHouse
		...
		TriggerClientEvent("pNotify:SendNotification", source, {
			text = 'Daj <strong class="amber-text">Klucz do domu</strong>',
			type = "success",
			timeout = 3000,
			layout = "bottomCenter",
			queue = "global"
		...
		TriggerClientEvent("pNotify:SendNotification", target, {
			text = 'Dostałeś <strong class="amber-text">klucz do domu</strong>',
			type = "success",
			timeout = 3000,
			layout = "bottomCenter",
			queue = "global"
...
ESX.RegisterServerCallback("esx_inventoryhud:getPlayerInventory1", function(source, cb, target, data)
	...
	if data == nil then
		...
		if data.vehicle == true then
			...
			local Vehicle_Key = MySQL.Sync.fetchAll('SELECT plate FROM owned_vehicles WHERE owner = @identifier', {
				['@identifier'] = xPlayer.identifier
			})
		...
		if data.house == true then
			...
			local Properties_Key = MySQL.Sync.fetchAll('SELECT name, id FROM owned_properties WHERE owner = @identifier', {
				['@identifier'] = xPlayer.identifier
			})
