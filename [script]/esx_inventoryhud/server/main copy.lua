...
AddEventHandler('esx_inventoryhud:getOwnerVehicle', function()
	...
	KeyItems = MySQL.Sync.fetchAll('SELECT plate FROM owned_vehicles WHERE owner = @identifier', {
...
AddEventHandler('esx_inventoryhud:getOwnerHouse', function()
	...
	HouseItems = MySQL.Sync.fetchAll('SELECT name, id FROM owned_properties WHERE owner = @identifier', {
		['@identifier'] = xPlayer.identifier
	})
